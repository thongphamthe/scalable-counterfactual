...
def run_one_experiment(num_sample):
    ...
    result_one_PCA = np.full((mm, 2), -1.00)
    for run_number in range(mm):
        with open('data_file/n_mixtureG/n_mixtureG_n_' + str(num_sample) + "_num_" + str(run_number) + '.pkl', 'rb') as f:
        ...
        result_one_PCA[run_number, 1] = emd_PCA
        result_one_PCA[run_number, 0] = time_pca
        print("n_samples: " + str(num_sample) + "- run: " + str(run_number))
    return result_one_PCA
...
import dill
with open('result_file/n_mixtureG_PCA_result.pkl', 'wb') as f:
