...
label_array = ["OT", "CiC", "Proposed", "Sinkhorn (10)", "Sinkhorn (30)", "Sinkhorn (90)"]
marker_array = ["o","p","s","<","^",">"]
...
for d in range(len(dim_array)):
    for i in range(len(run_vector)):
        if run_vector[i] in ["full","CiC"]:
            ...
            mean_array[d][i + 1] = np.mean(values_time)
            sd_array[d][i + 1] = np.std(values_time)
            mean_array_emd[d][i + 1] = np.mean(values_emd)
            sd_array_emd[d][i + 1] = np.std(values_emd)
